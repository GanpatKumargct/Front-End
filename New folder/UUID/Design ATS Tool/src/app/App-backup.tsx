...
type AppView = 'erp-access' | 'erp-modules' | 'ats' | 'purchase';
type ActiveTab = 'dashboard' | 'jobs' | 'candidates' | 'interviews' | 'analytics' | 'referrals' | 'settings';
type PurchaseTab =
  | 'dashboard'
  | 'requisition'
  | 'sourcing'
          ...
          <div className="grid grid-cols-3 gap-4">
            {departments.map((dept) => {
              ...
              const interviews = deptCandidates.filter(c =>
                ['PTC Interview', "Founder's Interview"].includes(c.status)
              ).length;
