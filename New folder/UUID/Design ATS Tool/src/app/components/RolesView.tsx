...
export function RolesView({ departmentId, departmentName, onBack, onRoleClick }: RolesViewProps) {
  const deptJobs = mockJobs.filter(j => j.departmentCode === departmentId && j.status === 'open');
  const deptCandidates = mockCandidates.filter(c => c.department === departmentId);
    ...
    <div className="p-6 space-y-6">
      ...
      {deptJobs.length === 0 ? (
        ...
        <div className="grid grid-cols-3 gap-4">
          {deptJobs.map((job) => {
            const roleCandidates = mockCandidates.filter(c => c.role === job.title && c.department === departmentId);
            const shortlisted = roleCandidates.filter(c =>
              ['Technical Interview', 'PTC Interview', 'Founders Interview'].includes(c.status)
            ).length;
            const interviews = roleCandidates.filter(c =>
              ['Technical Interview', 'PTC Interview', 'Founders Interview'].includes(c.status)
            ).length;
            const referrals = roleCandidates.filter(c => c.referralSource === 'Employee Referral').length;
