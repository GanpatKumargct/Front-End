...
export function DashboardPage({
  ...
  const activeRequisitions = mockJobs.filter(j => j.status === 'open').length;
  const interviewsInProgress = mockInterviews.filter(
    i => i.status === 'scheduled' || i.status === 'pending_feedback'
  ).length;
  const offersPending = mockCandidates.filter(c => c.status === "Founders' Interview").length;
  const hiresMade = mockCandidates.filter(c => c.status === 'Selected').length;
  ...
  const fitment = mockCandidates.filter(c => c.status === 'Fitment Evaluation').length;
  const technical = mockCandidates.filter(c => c.status === 'Technical Interview').length;
  const ptc = mockCandidates.filter(c => c.status === 'PTC Interview').length;
  const founders = mockCandidates.filter(c => c.status === "Founders' Interview").length;
  ...
  const deptHealth = departments.map(dept => {
    const deptCandidates = mockCandidates.filter(c => c.department === dept.code);
  ...
  // Aging requisitions
  const agingReqs = mockJobs
    .filter(j => j.status === 'open' && j.daysOpen > 90)
    .sort((a, b) => b.daysOpen - a.daysOpen)
    .slice(0, 5);

  // Pipeline health metrics
  const conversionRate = ((selected / totalApplicants) * 100).toFixed(1);
  const interviewRate = ((technical + ptc + founders) / totalApplicants * 100).toFixed(1);
  const offerAcceptanceRate = hiresMade > 0 ? ((hiresMade / offersPending) * 100).toFixed(0) : 0;
